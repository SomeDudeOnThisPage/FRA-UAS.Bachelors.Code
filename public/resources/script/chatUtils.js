...
export const setupChatEvents = (peer, room) => {
  ...
  $('#chat-input-submit').click(() => {
    submitMessage(peer, getPlayer(room.game.players, peer.id).name);
  });

  $('#chat-input').keypress((e) => {
    if (e.which === 13) { submitMessage(peer, getPlayer(room.game.players, peer.id).name); }
  });
  ...
  peer.on('chat', (packet) => {
    if (packet && packet.src) {
      ...
      const player = getPlayer(room.game.players, packet.src);
